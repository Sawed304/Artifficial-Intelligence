...
int main(int argc, char** argv)
{
    ...
    for (int i{ 0 }; i < CantGen; i++) {
        ...
        double currentBest = resultado.first.second;

        vectorMedias.push_back(resultado.first.first);
        vectorMejores.push_back(currentBest);
        ...
        if (i == 0) {
            ...
            if (abs(currentBest - prevBest) < TOL) {
                repCount++;
            }
            else {
                repCount = 1;
        ...
        if (repCount >= MAX_REP) {
            cout << "\n[STOP] El mejor valor se repitio "
                << MAX_REP << " veces consecutivas.\n";
            cout << "Convergencia alcanzada en la generacion " << i << endl;
            break;
        }
